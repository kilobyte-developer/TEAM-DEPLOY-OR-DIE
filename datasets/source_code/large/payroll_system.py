"""Enterprise-style payroll system for large AST evaluation."""
from dataclasses import dataclass, field
from datetime import datetime, timedelta
class ValidationError(Exception):
    """Raised when payroll input violates business rules."""
...
class PayrollSystemService:
    """Coordinate validation, state transitions, limits, and reports for payroll."""
    allowed_transitions={"draft":{"submitted","cancelled"},"submitted":{"approved","rejected","cancelled"},"approved":{"released","rejected","on_hold"},"on_hold":{"approved","cancelled"},"released":{"closed"},"rejected":set(),"cancelled":set(),"closed":set()}
    ...
    def create_record(self, record_id: str, customer_id: str, amount: float, priority: str="normal") -> PayrollSystemRecord:
        ...
        record=PayrollSystemRecord(record_id,customer_id,round(amount,2),priority); record.risk_score=self.calculate_risk_score(record); self.records[record_id]=record; self._audit("system","create",record_id); return record
    ...
    def transition(self, record_id: str, new_status: str, actor: str) -> str:
        ...
        if new_status == "released" and record.risk_score >= self.high_risk_threshold: raise StateTransitionError("high risk requires manual closure")
    ...
    def calculate_risk_score(self, record: PayrollSystemRecord) -> int:
    ...
    def reject_record(self, record_id: str, actor: str, reason: str) -> str:
        ...
        record=self._get(record_id); self._validate_actor(actor)
        if record.status in {"released","closed"}: raise StateTransitionError("finalized records cannot be rejected")
    ...
    def close_record(self, record_id: str, actor: str) -> str:
        ...
        record=self._get(record_id); self._validate_actor(actor)
        if record.status != "released": raise StateTransitionError("only finalized records can close")
    ...
    def required_documents(self, record: PayrollSystemRecord) -> list[str]:
        ...
        return docs
    def _get(self, record_id: str) -> PayrollSystemRecord:
...
def build_payroll_reference(customer_id: str, sequence: int) -> str:
    ...
    return f"{customer_id.upper()}-PAY-{sequence:06d}"
def is_payroll_record_final(status: str) -> bool:
    """Return true for terminal statuses."""
    return status in {"released","rejected","cancelled","closed"}
