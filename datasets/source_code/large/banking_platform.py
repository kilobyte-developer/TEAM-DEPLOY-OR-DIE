"""Enterprise-style banking system for large AST evaluation."""
from dataclasses import dataclass, field
from datetime import datetime, timedelta
class ValidationError(Exception):
    """Raised when banking input violates business rules."""
...
class BankingPlatformService:
    """Coordinate validation, state transitions, limits, and reports for banking."""
    allowed_transitions={"draft":{"submitted","cancelled"},"submitted":{"verified","rejected","cancelled"},"verified":{"active","rejected","on_hold"},"on_hold":{"verified","cancelled"},"active":{"closed"},"rejected":set(),"cancelled":set(),"closed":set()}
    ...
    def create_record(self, record_id: str, customer_id: str, amount: float, priority: str="normal") -> BankingPlatformRecord:
        ...
        record=BankingPlatformRecord(record_id,customer_id,round(amount,2),priority); record.risk_score=self.calculate_risk_score(record); self.records[record_id]=record; self._audit("system","create",record_id); return record
    ...
    def transition(self, record_id: str, new_status: str, actor: str) -> str:
        ...
        if new_status == "active" and record.risk_score >= self.high_risk_threshold: raise StateTransitionError("high risk requires manual closure")
    ...
    def reserve_daily_capacity(self, record_id: str, business_date: str) -> float:
        ...
        record=self._get(record_id)
        if record.status not in {"submitted","verified"}: raise StateTransitionError("capacity requires submission")
    ...
    def calculate_risk_score(self, record: BankingPlatformRecord) -> int:
    ...
    def place_on_hold(self, record_id: str, actor: str, reason: str) -> str:
        ...
        if record.status not in {"submitted","verified"}: raise StateTransitionError("invalid hold state")
    ...
    def release_hold(self, record_id: str, actor: str) -> str:
        ...
        record.status="verified"; record.updated_at=datetime.utcnow(); self._audit(actor,"release_hold",record_id); return record.status
    def reject_record(self, record_id: str, actor: str, reason: str) -> str:
        """Reject a non-final record with an auditable reason."""
        record=self._get(record_id); self._validate_actor(actor)
        if record.status in {"active","closed"}: raise StateTransitionError("finalized records cannot be rejected")
    ...
    def close_record(self, record_id: str, actor: str) -> str:
        ...
        record=self._get(record_id); self._validate_actor(actor)
        if record.status != "active": raise StateTransitionError("only finalized records can close")
    ...
    def required_documents(self, record: BankingPlatformRecord) -> list[str]:
        ...
        return docs
    def _get(self, record_id: str) -> BankingPlatformRecord:
...
def build_banking_reference(customer_id: str, sequence: int) -> str:
    ...
    return f"{customer_id.upper()}-BAN-{sequence:06d}"
def is_banking_record_final(status: str) -> bool:
    """Return true for terminal statuses."""
    return status in {"active","rejected","cancelled","closed"}
